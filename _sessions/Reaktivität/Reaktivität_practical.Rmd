...
4. Lasse die App nun einmal laufen mit `shinyApp(ui, server)`. Alles sollte funktionieren wie zuvor. Dieses Beispiel illustriert, dass inputs in Objekten speichern kann, aber nur wenn man sie z.B. als `reactive speichert`. Versuche einmal die App laufen, wenn du `reactive` entfernst und das Ergebnis von `paste` direkt zuweist. Natürlich musst du dann auch die Klammern von `gruss_text` im renderText entfernen, da `gruss_text` nun kein `reactive` mehr ist. Funnktionert die App nun immer noch?
...
1. Kopiere den Code der letzten Iteration der "Wie heisst Du?" App in ein neues Skript (oder ändere einfach die vorherige Version). Nun wollen wir eine weitere Mitteilungen ergänzen, die uns rückmeldet, dass ein Gruss ausgerichtet wurde. Verwende hierzu `observeEvent` innerhalb des `servers`. Als erstes Argument stelle den Input bereit `input$name`. Der Input kontrollier dann den Code, den wir gleich für das zweite Argument bereitstellen ausgeführt in dem Sinne, dass anschliessende Code jedes mal ausgeführt wird wenn `input$name` validiert wird. Als Code für das zweite Argument weise `message("Gruss ausgerichtet")` zu, was zu einer Nachricht in der Console führen sollte, jedes mal wenn der geändert, bzw validiert wird Input bereitgestellt wird.    
...
3. Die Konstruktion mit `observeEvent` und `req` ist nützlich, um streng zu kontrollieren, wann ein bestimmter laufen soll. Wir können dies zum Beispiel auch verwenden, um zu verhinden, dass `"Hallo *"` schon dort steht bevor wir etwas eingegeben haben. Dafür müssen wir jedoch zuerst `gruss_text` in ein anderes reaktives Format ändern und zwar in einen `reactiveVal`. `reactiveVal` erlauben die Definition eines noch undefiniertes Wertes, der zudem im Vergleich zu normalen `reactive` im Code überschrieben werden kann. Definiere zunächst `gruss_text` als leeren `reactiveVal`. 
...
server <- function(input, output, session) {
  ...
  output$XX <- renderText(XX())
  observeEvent(XX(XX), {
    reactiveVal(paste0("Hallo ", input$name, "!"))
    message("Gruss ausgerichtet")
    })
  }
...
3. Da die Daten nun in einem `reactive` sind, müssen wir den Code von `linien_plot` anpassen. Anstatt innerhalb der Plotcodes die Auswhal der Daten vorzunehmen, soll nun das `reactive` die Daten bereitsstellen. Entferne also den relevanten `dplyr` Code zu Beginn plots und ersetze ihn durch das `reactive`. Denke dran `reactive`s sind wie Funktionen und brauchen daher Klammern. 
